...
# Export des tables
psql -d pano -c "\COPY (SELECT id, replace(code::text,'\"',''), etat, azimuth, replace(rattachements::text,'\"',''), producteur, commentaire, ST_AsText(geometrie) AS wkt FROM pano_valide) TO pano_valide_complet.csv CSV HEADER DELIMITER ','"
